 ...
 * 2. @WebServlet(요청 주소) 어노테이션을 이용하는 방법  
 * 
 * */
...
public class ExampleController1 extends HttpServlet{
	...
	// doGet() 메서드
	//-> GET 방식 요청을 처리하는 메서드
	// 	 HttpServlet 의 메서드를 오버라이딩하여 사용
	
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		
		// HttpServletRequest
		// - 클라이언트 요청 시 생성되는 객체로 요청 시 함께 전달된 데이터,
		//	 요청한 클라이언트의 정보, 유연한 요청 처리를 하기 위한 객체를 제공함
		
		// HttpServletResponse
		// - 클라이언트 요청 시 생성되는 객체로,
		// 	 서버가 클라이언트에게 응답하기 위한 방법을 제공함
		// 	 이때 서버 -> 클라이언트로 연결된 출력용 스트림 제공(PrintWriter)
		
		System.out.println("--- 이름, 나이를 입력받아 처리하는 코드 ---");
		
		// 요청 시 입력된 이름, 나이를 전달 받아오기
		
		// Parameter : 매개변수 == 다른 곳의 값을 전달받아올 때 사용
		...
		// !!!!HTML에서 얻어오는 모든 값은 String으로 넘어옴 !!!!
		...
		// text/html : 글자로 된 html 형식의 문서
		// charset : 문자 인코팅 지정 속성
		...
		// 응답처리 4 : 출력 스트림을 이용해 클라이언트에게 HTML코드 출력(응답)
