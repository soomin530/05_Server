...
<body>
	<h3>1. 변수 선언(== 속성 추가) - c:set 태그 이용</h3>
    ...
    * 주의사항 : else 구문이 없다!
	...
	<%-- step 속성 값은 양수만 가능! (음수 X) --%>
	
	<c:forEach var="i" begin="1" end="6" step="1">
		<h${7-i}>h${7-1} 태그 입니다</h${7-i}>  
	</c:forEach>
	...
	<ul>
		<c:forEach var="name" items="${nameList}" varStatus="vs"> 
			...
			<c:if test="${vs.first}"> <%-- 반복문의 첫 번째 순회일 때 --%>
				<li>---------------시작---------------</li>
			</c:if>
