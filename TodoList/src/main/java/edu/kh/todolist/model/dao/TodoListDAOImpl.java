...
public class TodoListDAOImpl implements TodoListDAO {
	...
	public TodoListDAOImpl() throws Exception{
		...
		if(!file.exists()) { // 파일이 존재하지 않는다면
			...
			todoList.add(new Todo("JSP 공부", "수업 내용 복습하기 2"));
			todoList.add(new Todo("TodoList 만들기", "수업 내용 복습하기 3")); 
			
		} else { // 파일이 이미 존재한다면
			// 외부 파일에 작성된 List<Todo> 객체를 입력 받아 todoList에 대입
	...
	public int todoAdd(Todo todo) throws Exception{
		if(todoList.add(todo)) {
			saveFile(); // 외부 파일로 출력
			return todoList.size()-1; // 성공 시 마지막 인덱스 번호 반환
		}
